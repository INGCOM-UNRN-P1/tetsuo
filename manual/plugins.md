---
title: "Guía de Extensión y Creación de Plugins: tetsuo"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de tetsuo"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-tetsuo-plugins)=
# Guía de Extensión y Plugins: tetsuo

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`tetsuo`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-tetsuo-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`tetsuo` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para tetsuo`.
- **Punto de Entrada Oficial**: `tetsuo.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-tetsuo-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para tetsuo."""
    name = "custom_rule"
...
Para que `tetsuo` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."tetsuo.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `tetsuo` la reconozca:
...
(manual-tetsuo-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `tetsuo` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = tetsuo.ejecutar_analisis(
...
(manual-tetsuo-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `tetsuo` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar tetsuo
        run: pip install -e ./tetsuo
        
      - name: Ejecutar Auditoría
        run: tetsuo check src/ include/ --json > reporte.json
````

---

(manual-tetsuo-plugins-ejercicios)=
